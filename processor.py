...
def count_frequency_of_songs_by_country(file) -> dict:
    country_songs = {}
    for line in file:
...
def save(values: List[str], file_name: str, output_path: Path) -> None:
    with open(output_path / get_filename(file_name), "w") as result_file:
        result_file.writelines(values)


def get_filename(input_file_name: str) -> str:
...
def filter_top_n_songs(n: int, count_dict: dict) -> List[str]:
    result = []
...
def get_top_songs(n: int,
    ...
    try:
        with open(data_path / 'input' / input_file) as f:
            data = count_frequency_of_songs_by_country(f)
            top_n_songs = filter_top_n_songs(n=n, count_dict=data)
            save(top_n_songs, input_file, output_path=output_path)
